             ...
             where Dado : IComparable<Dado>, IRegistro, new()
{
    ...
    public void LerArquivoDeRegistros(string nomeArquivo)
    {
        ...
        int posicaoFinal = (int)origem.Length / dado.TamRegistro - 1;
    ...
    {
        // melhores valores que encontramos
        // ainda assim, a exibição ficou um pouco
        // prejudicada :( acreditamos que tem
        // a ver com a quntidade de nós
        DesenharArvore(true, raiz, x, y, 80, 1.55, 250, g);
    }
                ...
                double comprimento, Graphics g)
    {
        ...
        if (raiz != null)
        {
            var caneta = new Pen(Color.Black);
            ...
            DesenharArvore(false, raiz.Dir, xf, yf,
                           Math.PI / 2 - incremento,
                           incremento * 0.60, comprimento * 0.8, g);
            SolidBrush preenchimento = new SolidBrush(Color.LightGray);
    ...
    private NoArvore<Dado> InserirBalanceado(Dado item, NoArvore<Dado> no)
    {
        ...
        {
            if (item.CompareTo(no.Info) < 0)
            {
                no.Esq = InserirBalanceado(item, no.Esq);
                if (Math.Abs(GetAltura(no.Esq) - GetAltura(no.Dir)) == 2)
                    if (item.CompareTo(no.Esq.Info) < 0)
              ...
              if (item.CompareTo(no.Info) > 0)
            {
                no.Dir = InserirBalanceado(item, no.Dir);
                if (Math.Abs(GetAltura(no.Dir) - GetAltura(no.Esq)) == 2)
                    if (item.CompareTo(no.Dir.Info) > 0)
    ...
    private NoArvore<Dado> RotacaoSimplesEsq(NoArvore<Dado> no)
    {
    ...
    private NoArvore<Dado> RotacaoSimplesDir(NoArvore<Dado> no)
    {
    ...
    private NoArvore<Dado> RotacaoDuplaEsq(NoArvore<Dado> no)
    {
        no.Esq = RotacaoSimplesDir(no.Esq);
        return RotacaoSimplesEsq(no);
    }

    private NoArvore<Dado> RotacaoDuplaDir(NoArvore<Dado> no)
    {
        no.Dir = RotacaoSimplesEsq(no.Dir);
    ...
    private bool ExisteRec(NoArvore<Dado> local, Dado procurado)
    {
        ...
        if (procurado.CompareTo(local.Info) < 0)
            return ExisteRec(local.Esq, procurado);
        else
            return ExisteRec(local.Dir, procurado);
    }
